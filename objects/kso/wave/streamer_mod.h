#ifndef STREAMER_MOD_H
#define STREAMER_MOD_H
...
typedef struct {
    ...
    volatile int do_seek;
    volatile uint32_t seek_pos;
    volatile uint32_t initial_pos;
    char* filename;
    int offset;
    fbuffer fbuff0;
    fbuffer fbuff1;
    ...
    while (!chThdShouldTerminate()) {
        ...
        do {
            ...
            if (s->pingpong == OPEN) {
                ...
                if (!s->do_seek) {
                    err = f_read(&s->f, s->fbuff0.i8buff, SDREADFILEPINGPONGSIZE * 4, &bytes_read);
                    if (err) report_fatfs_error(err, &s->filename[0]);
                    s->offset = 0;
                    chSysDisable();
                    if (s->pingpong != CLOSING) s->pingpong = PLAYA_READB;
                    chSysEnable();
                }
                busy = 1;
            }
                ...
                s->offset = 0;
                chSysDisable();
                if (s->pingpong != CLOSING) s->pingpong = RECB;
                chSysEnable();
            ...
            else if (s->do_seek && (s->pingpong != CLOSED)) {
                s->pingpong = SEEKING;
                err = f_lseek(&s->f, s->seek_pos);
                if (err) report_fatfs_error(err, &s->filename[0]);
                err = f_read(&s->f, s->fbuff0.i8buff, SDREADFILEPINGPONGSIZE * 4, &bytes_read);
                if (err) report_fatfs_error(err, &s->filename[0]);
                s->offset = 0;
            ...
            if (s->pingpong == PLAYB_READA) {
                ...
                if (f_tell(&s->f) + SDREADFILEPINGPONGSIZE * 4 < f_size(&s->f)) {
                    err = f_read(&s->f, s->fbuff1.i8buff, SDREADFILEPINGPONGSIZE * 4, &bytes_read);
                    if (err) report_fatfs_error(err, &s->filename[0]);
                    chSysDisable();
                    if (s->pingpong == PLAYA_READB) s->pingpong = PLAYA;
                    chSysEnable();
                    if ((err != FR_OK) || (bytes_read != SDREADFILEPINGPONGSIZE * 4)) {
                        s->pingpong = CLOSING;
                    }
                ...
                err = f_write(&s->f, s->fbuff0.i8buff, SDREADFILEPINGPONGSIZE * 4, &bytes_read);
                chSysDisable();
                ...
                err = f_write(&s->f, s->fbuff1.i8buff, SDREADFILEPINGPONGSIZE * 4, &bytes_read);
                chSysDisable();
                if (s->pingpong == RECA_WRITEB) s->pingpong = RECA;
                chSysEnable();
                if (err != FR_OK) {
                    s->pingpong = CLOSINGREC;
                }
        ...
        if (!chThdShouldTerminate()) chEvtWaitAnyTimeout((eventmask_t) 1, 10);
    }
    sdReadFilePingpong *s = (sdReadFilePingpong *) arg;
...
int16_t * sdWriteStream(sdReadFilePingpong *s) {
    ...
    if ((s->pingpong == RECB_WRITEA) || (s->pingpong == RECB)) {
        p = &s->fbuff1.i16buff[s->offset];
        if (s->offset == 0) {
            if (s->pingpong != CLOSINGREC) s->pingpong = RECB_WRITEA;
            chEvtSignal(s->pThreadSD, (eventmask_t) 1);
        }
        s->offset += BUFSIZE;
        if (s->offset == SDREADFILEPINGPONGSIZE * 2) {
            if (s->pingpong != CLOSINGREC) s->pingpong = RECA;
            s->offset = 0;
        }
    }
        ...
        p = &s->fbuff0.i16buff[s->offset];
        if (s->offset == 0) {
            if (s->pingpong != CLOSINGREC) s->pingpong = RECA_WRITEB;
            chEvtSignal(s->pThreadSD, (eventmask_t) 1);
        }
        s->offset += BUFSIZE;
        if (s->offset == SDREADFILEPINGPONGSIZE * 2) {
            if (s->pingpong != CLOSINGREC) s->pingpong = RECB;
            s->offset = 0;
        }
    }
    else {
